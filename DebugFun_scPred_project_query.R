project_query <- function (new, reference, max.iter.harmony = 20, recompute_alignment = TRUE,
                           seed = 66, ...)
{
  ...
  if (alignment) {
    ...
    set.seed(seed)
    harmony_embeddings <- HarmonyMatrix(eigenspace, meta_data,
    ...
    colnames(new_embeddings_aligned) <- gsub("scpred_",
                                             spmodel@reduction_key, colnames(new_embeddings_aligned))
  }
