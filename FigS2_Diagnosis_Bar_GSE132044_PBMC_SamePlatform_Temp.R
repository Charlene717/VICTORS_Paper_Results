...
# if (Set_Dataset == "HLCA_core") {
#   TargetCell_data <- selected_data %>%
#     filter(Actual_Cell_Type %in% Mislabel_CellType)  # 筛选Actual_Cell_Type在Mislabel_CellType中的记录
# }
...
type_count <- TargetCell_data %>%
  group_by(Actual_Cell_Type, FileID) %>%
  summarise(Count = n())
...
max_diff <- Summary.df2 %>%
  ...
  pivot_wider(names_from = Method, values_from = Value) %>%
  mutate(Difference = !!sym(Victors_Method) - !!sym(Set_Method)) %>%
  left_join(type_count, by = c("Actual_Cell_Type", "FileID")) %>%
  # 当 Set_Dataset 为 "HLCA_core" 时，筛选出在 valid_cell_types 中且 Count 大于 100 的 Actual_Cell_Type
  filter(if (Set_Dataset == "HLCA_core") {
    Actual_Cell_Type %in% all_data$Mislabel_CellType %>% unique() &
      # Actual_Cell_Type != "Suprabasal" &
      # Actual_Cell_Type != "Nasal club cells" &
      Count > 100
  } else TRUE) %>%
