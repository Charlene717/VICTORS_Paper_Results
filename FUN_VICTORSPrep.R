...
#################################################################################
## ClassR
setClass("VICTORS", representation(pvar = "character",
                                  metadata = "data.frame",
                                  features = "list",
...
#################################################################################
## generics.R
setGeneric("get_VICTORS", def = function(object) {
  standardGeneric("get_VICTORS")
})
...
getFeatureSpace <- function(object, pvar, correction = "fdr", sig = 1, reduction = "pca"){
  ...
  spmodel <- new("VICTORS", metadata = data.frame(pvar = classes))
...
################################################################################
## methods.R
setMethod("get_VICTORS",
          signature("Seurat"),
          function(object){
            VICTORS <- object@misc$VICTORS
            if(is.null(VICTORS))
              stop("No VICTORS model is stored in this Seurat object")
            else
              VICTORS
          })
...
setMethod("get_metadata",
          signature("Seurat"),
          function(object){
            get_metadata(get_VICTORS(object))
          })
...
project_query <- function(new,
  ...
  # Validate if provided object is an VICTORS object
  if(!(is(reference, "Seurat") | is(reference, "VICTORS"))) stop("'object' must be of class 'VICTORS' or 'Seurat'")

  if(is(reference, "Seurat")){
    spmodel <- reference@misc$VICTORS
  }else{
    spmodel <- reference
  }
  ...
  if("VICTORS" %in% names(new@reductions)){
  ...
  if(alignment){
    ...
    new_embeddings_aligned <- Embeddings(new, reduction = "VICTORS")
    colnames(new_embeddings_aligned) <- gsub("VICTORS_", spmodel@reduction_key, colnames(new_embeddings_aligned))
  }


  rownames(new_embeddings_aligned) <- gsub("^new_", "", rownames(new_embeddings_aligned))
  new@reductions[["VICTORS"]] <- CreateDimReducObject(embeddings = new_embeddings_aligned,
                                                     key = "VICTORS_",
                                                     assay = DefaultAssay(object = new))
  if(recompute_alignment){

    rownames(new_embeddings) <- gsub("^new_", "", rownames(new_embeddings))
    new@reductions[["VICTORS_projection"]] <- CreateDimReducObject(embeddings = new_embeddings,
                                                                  key = "Projection_",
                                                                  assay = DefaultAssay(object = new))
  }
...
VICTORSPrep <- function(new,
  ...
  new_embeddings_aligned <- Embeddings(new[["VICTORS"]])
  ...
  names(res) <- .make_names(paste0("VICTORS_", names(res)))
...
trainModel <- function(object,
  ...
  # Check class
  if(!is(object, "Seurat") | is(object, "VICTORS")){
    stop("object must be 'Seurat' or 'VICTORS'")
  }


  if(is(object, "Seurat")){
    seurat_object <- object
    object <- get_VICTORS(object)
  ...
  if(object_class == "Seurat"){
    seurat_object@misc$VICTORS <- object
    seurat_object
